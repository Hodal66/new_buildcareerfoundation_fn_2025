/* eslint-disable react/no-unknown-property */

import { HeaderComponent } from "../components/HeaderComponent";
import FooterComponentOne from "../components/FooterComponentOne";
import { useFormik } from "formik";
import * as Yup from "yup";
import { gql, useMutation } from "@apollo/client";
// import { useState } from "react";
import { useNavigate } from "react-router";
import Paragraph1 from "../components/Headings/Paragraph1";
import styles from "../styles";
import MainHeading1 from "../components/Headings/MainHeading1";

const LOG_IN = gql`
  mutation Mutation($email: String!, $password: String!) {
    login_user(email: $email, password: $password) {
      token
      user {
        _id
        email
        firstName
        secondName
        password
        posts
      }
    }
  }
`;
/* eslint-disable react/no-unescaped-entities */
export const LoginIn = () => {
  // eslint-disable-next-line no-unused-vars
  // const [errorMessage, setErrorMessage] = useState("");
  const initialData = {
    firstName: "",
    secondName: "",
    email: "",
    password: "",
  };
  const navigate = useNavigate();
  // eslint-disable-next-line no-unused-vars
  const [logiInUserFunc, { data, loading, error }] = useMutation(LOG_IN, {
    onCompleted: async () => {
      localStorage.setItem("userToken",data);
      navigate(`/admin/overview`);
    },
    onError: (errors) => {
      console.log(errors);
      // setErrorMessage()
    },
  });
  //...
  const validate = Yup.object({
    email: Yup.string().required("Email is Required, can not be empty"),
    password: Yup.string().required("Password is Required, can not be empty"),
  });
  let loginResponse;
  const onSubmit = async (values, { resetForm }) => {
    const { email, password } = values;
    try {
      loginResponse = await logiInUserFunc({
        variables: { email: email, password: password },
      });
      // if (error) {
      console.log(loginResponse);
      // }
      resetForm({});
    } catch (error) {
      console.log(error);
    }
  };
  const formik = useFormik({
    initialValues: initialData,
    onSubmit: onSubmit,
    validationSchema: validate,
  });
  return (
    <div>
      <HeaderComponent />
      {/* Header ends here ! */}
      <div className={`${styles.blueGradient} flex justify-center font-montiseramwa leading-relaxed text-blackPhant`}>
        <div className="w-auto text-center h-auto my-16">
          <div className="w-full bg-white h-full  p-8   text-start rounded-roundedBox">
            <MainHeading1 Title={"Welcome to Login"}/>
            <div className="text-btnSize mb-[32px]">
              <div className="flex gap-2">
                <Paragraph1 Title={"Note that this is dedicated only for"} />
                <span className="text-green-400 font-semibold">Admin</span>
              </div>
            </div>
            <div >
              <form onSubmit={formik.handleSubmit} role="form">
                <div className="text-guidanceMessageColorOnForm text-btnSize">
                  Fields marked with an <span className="text-red-600">*</span>{" "}
                  are required.....
                </div>
                <div className="pb-[30px]">

                  <div className="mb-6">
                    <label
                      for="email"
                      className="block mb-2 text-lg font-bold text-guidanceMessageColorOnForm"
                    >
                      Email<span className="text-red-600">*</span>
                    </label>
                    <input
                      type="email"
                      onChange={formik.handleChange}
                      value={formik.values.email}
                      onBlur={formik.handleBlur}
                      name="email"
                      id="email"
                      className="bg-bgInputColor border border-borderInputColor text-gray-900 focus:outline-none focus:border-gray-500 text-sm block w-full p-2.5"
                      placeholder="example@gmail.com"
                      required
                    />
                    {/* conditional rendering of the error message for validating the name input field */}
                    {formik.touched.email && formik.errors.email ? (
                      <div className="text-errorText">
                        {formik.errors.email}
                      </div>
                    ) : null}
                  </div>
                  <div className="mb-6">
                    <label
                      for="password"
                      className="block mb-2 text-lg font-bold text-guidanceMessageColorOnForm"
                    >
                      Password<span className="text-red-600">*</span>
                    </label>
                    <input
                      type="password"
                      onChange={formik.handleChange}
                      value={formik.values.password}
                      onBlur={formik.handleBlur}
                      name="password"
                      id="password"
                      className="bg-bgInputColor border border-borderInputColor text-gray-900 focus:outline-none focus:border-gray-500 text-sm block w-full p-2.5"
                      placeholder="Enter password"
                      required
                    />
                    {/* conditional rendering of the error message for validating the name input field */}
                    {formik.touched.password && formik.errors.password ? (
                      <div className="text-errorText">
                        {formik.errors.password}
                      </div>
                    ) : null}
                  </div>
                  {loading && (
                    <div className="  rounded-md text-black flex h-[30px] p-6 items-center text-[20px]">
                      <svg
                        aria-hidden="true"
                        className="w-6 h-6 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                        viewBox="0 0 100 101"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                          fill="currentColor"
                        />
                        <path
                          d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                          fill="currentFill"
                        />
                      </svg>
                      <div className="px-4">
                        <p>Loging in ...</p>
                      </div>
                    </div>
                  )}
                  <button
                    type="submit"
                    className="text-white bg-blue-700 hover:bg-gray-400 hover:text-gray-600 focus:outline-none font-medium text-sm w-full sm:w-auto px-5 py-2.5 text-center"
                  >
                    Login
                  </button>
                </div>
              </form>
            </div>
          </div>
        </div>
      </div>

      <FooterComponentOne />
    </div>
  );
};
