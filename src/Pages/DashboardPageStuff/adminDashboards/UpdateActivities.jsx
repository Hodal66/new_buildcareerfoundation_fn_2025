...
//     arrayOfImages: Yup.mixed(),
//     contentSections: Yup.array().of(
//       Yup.object().shape({
//         sectionTitle: Yup.string().required("Section title required"),
//         paragraph1: Yup.string().required("Content required"),
//       })
//     ),
//   });
...
//         if (values.arrayOfImages.length > 0) {
//           setCurrentStep("uploading");
//           const formData = new FormData();
//           [...values.arrayOfImages].forEach((file) =>
...
//         setTimeout(() => navigate("/admin/overview"), 4500);
//       } catch (err) {
//         console.error("Update error:", err);
//         toast.error(
//           <div className="flex items-center space-x-2">
//             <AlertCircle className="h-4 w-4" />
//             <span>{err.message || "Failed to update post"}</span>
//           </div>,
...
//       const initialImages = post.image_urls || [];
      
//       setExistingImages(initialImages);
//       setImagePreview(initialImages.map(img => img.url));

//       formik.setValues({
//         postTitle: post.title || "",
//         contentTitle: post.content || "",
//         contentSections: post.contentSections?.length > 0 
...
//     // Create new previews while maintaining existing ones
//     const newPreviews = [];
//     for (let i = 0; i < files.length; i++) {
//       newPreviews.push(URL.createObjectURL(files[i]));
//     }
//     setImagePreview([...existingImages.map(img => img.url), ...newPreviews]);
//   };

//   const getLoadingMessage = () => {
//     switch (currentStep) {
//       case "uploading":
//         return "Uploading new images...";
//       case "saving":
...
//       <p className="mt-2">Error loading post data</p>
//     </div>
//   );
...
//       >
//         {/* Form fields same as create component but with existing data */}
//         {/* Post Title */}
...
//                 {...formik.getFieldProps(`contentSections[${index}].sectionTitle`)}
//                 className="w-full border px-3 py-2 rounded"
//               />
//               {formik.errors.contentSections?.[index]?.sectionTitle && (
...
//                 rows="2"
//                 {...formik.getFieldProps(`contentSections[${index}].paragraph1`)}
//                 className="w-full border px-3 py-2 rounded"
//               />
//               {formik.errors.contentSections?.[index]?.paragraph1 && (
//                 <div className="text-red-500 text-sm">
//                   {formik.errors.contentSections[index].paragraph1}
//                 </div>
//               )}
...
//                 rows="2"
//                 {...formik.getFieldProps(`contentSections[${index}].paragraph2`)}
//                 className="w-full border px-3 py-2 rounded"
//               />
...
//           <select
//             name="postCategory"
//             {...formik.getFieldProps("postCategory")}
...
//         {/* Image Upload */}
//         <div className="md:col-span-2">
//           <label className="block mb-1 font-medium">Update Images</label>
...
//             <label
//               htmlFor="fileInput"
//               className="cursor-pointer block text-center p-4"
//             >
...
//             {/* New Image Previews */}
//             {imagePreview.slice(existingImages.length).map((src, index) => (
//               <div key={`new-${index}`} className="relative mt-4">
//                 <img
//                   src={src}
//                   alt={`New Preview ${index + 1}`}
//                   className="h-24 w-full object-cover rounded border"
//                 />
//                 <span className="absolute top-0 right-0 bg-blue-600 text-white text-xs px-1 rounded-bl">
//                   New {index + 1}
//                 </span>
//               </div>
//             ))}
